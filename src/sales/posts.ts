...
import { sanitizeTelegramHtml, visibleLength } from "../lib/tghtml";
...
export type PostButton = { text: string; type: "plan" | "planlar" | "bot" | "free_channel" | "support" | "url"; value?: string };
export type PostOptions = { silent?: boolean; protect?: boolean; pin?: boolean; noPreview?: boolean };
export type PostInput = {
  id?: number; channel: "free" | "vip"; title: string; tags: string[]; text: string; mediaType: "photo" | "video" | null; mediaPath: string | null;
  buttons: PostButton[][]; options: PostOptions; scheduledAt: string | null; templateId?: number | null;
};
export type PostRow = {
  ...
  buttons: PostButton[][]; options: PostOptions; status: "draft" | "scheduled" | "sending" | "sent" | "failed"; scheduled_at: string | null; sent_at: string | null;
...
export function validatePost(raw: Partial<PostInput>, { allowClaims = false } = {}): PostInput {
  ...
  const mediaType = raw.mediaType === "photo" || raw.mediaType === "video" ? raw.mediaType : null;
  ...
  if (!text && !mediaPath) problems.push("Metin ya da görsel gerekli.");
  const max = mediaPath ? CAPTION_MAX : TEXT_MAX;
  if (visibleLength(text) > max) problems.push(`Metin en fazla ${max} karakter olabilir (görselli paylaşımda ${CAPTION_MAX}).`);
  ...
  for (const row of Array.isArray(raw.buttons) ? raw.buttons.slice(0, 8) : []) {
    ...
    for (const b of Array.isArray(row) ? row.slice(0, 3) : []) {
      ...
      cleanRow.push({ text: label, type, value: type === "plan" || type === "url" ? value : undefined });
    }
    if (cleanRow.length) buttons.push(cleanRow);
  }
  const o = (raw.options ?? {}) as PostOptions;
  const options: PostOptions = { silent: Boolean(o.silent), protect: Boolean(o.protect), pin: Boolean(o.pin), noPreview: o.noPreview !== false };
  ...
  if (!allowClaims) {
    const claims = findForbiddenClaims(text.replace(/<[^>]+>/g, " ")).filter((c) => c !== "raw_link" && c !== "invented_promotion");
  ...
  return { id: raw.id, channel, title: title || (text.replace(/<[^>]+>/g, "").split("\n")[0] ?? "").slice(0, 60) || "Paylaşım", tags, text, mediaType, mediaPath, buttons, options, scheduledAt, templateId: raw.templateId ?? null };
}
...
export async function sendPost(id: number): Promise<PostRow> {
  ...
  if (post.content_purged) throw new PostError("Bu paylaşımın metni silinmiş (gönderimden 1 saat sonra silinir); kopyalayıp yeniden oluşturun.");
  ...
  const common = { chat_id: chatId, parse_mode: "HTML", disable_notification: Boolean(post.options?.silent), protect_content: Boolean(post.options?.protect), ...(keyboard ? { reply_markup: keyboard } : {}) };
  try {
    let sent: { message_id: number };
    if (post.media_type && post.media_path) {
      const method = post.media_type === "photo" ? "sendPhoto" : "sendVideo";
      sent = await tg<{ message_id: number }>(method, { ...common, [post.media_type]: mediaUrl(post.media_path), ...(text ? { caption: text } : {}) });
    } else {
      sent = await tg<{ message_id: number }>("sendMessage", { ...common, text, link_preview_options: { is_disabled: post.options?.noPreview !== false } });
    }
...
export async function editInChannel(id: number, input: PostInput): Promise<void> {
  ...
  if (post.media_type) await tg("editMessageCaption", { ...base, caption: input.text });
  else await tg("editMessageText", { ...base, text: input.text, link_preview_options: { is_disabled: input.options.noPreview !== false } });
...
export async function savePost(input: PostInput, mode: "draft" | "schedule" | "send"): Promise<PostRow> {
  ...
  const values = {
    channel: input.channel, title: input.title, tags: input.tags, text: input.text, media_type: input.mediaType, media_path: input.mediaPath, buttons: input.buttons, options: input.options,
...
export async function duplicatePost(id: number): Promise<PostRow> {
  ...
  const { data: copy, error } = await db().from("channel_posts").insert({ channel: post.channel, title: `${post.title} (kopya)`.slice(0, 120), tags: post.tags, text: post.text, media_type: post.media_type, media_path: post.media_path, buttons: post.buttons, options: post.options, status: "draft", template_id: post.template_id }).select("*").single();
  if (error) throw new PostError(error.message);
...
export async function saveTemplate(input: PostInput & { templateId?: number | null }): Promise<void> {
  const values = { channel: input.channel, title: input.title, tags: input.tags, text: input.text, media_type: input.mediaType, media_path: input.mediaPath, buttons: input.buttons, options: input.options, updated_at: new Date().toISOString() };
  if (!values.text && !values.media_path) throw new PostError("Şablon boş olamaz.");
